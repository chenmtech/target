...
#define HAL_ADC_STSEL_EXT   0x00    /* External Trigger on P2.0*/
...
#define HAL_ADC_STSEL       HAL_ADC_STSEL_ST      /* 采样启动方式：由ADCCON1.ST触发   */
#define HAL_ADC_RAND_GEN    HAL_ADC_RAND_STOP     /* 停止生成随机数  */
#define HAL_ADC_REF_VOLT    HAL_ADC_REF_AVDD      /* 用外部电源电压AVDD作为参考电压  */
#define HAL_ADC_DEC_RATE    HAL_ADC_DEC_064       /* 抽取率：64，量化分辨率为8bits */
#define HAL_ADC_SCHN        HAL_ADC_CHN_VDD3      /* 通道：AVDD/3 */
#define HAL_ADC_ECHN        HAL_ADC_CHN_GND       /* 通道：GND  */
...
{
#if (HAL_ADC == TRUE)
  adcRef = HAL_ADC_REF_VOLT;    //默认的参考电压：AVDD
#endif
}
...
extern void HalAdcEnableTemp()
{
  TR0 = 0x01;
  ATEST = 0x01;
}

extern void HalAdcDisableTemp()
{
  ATEST = 0x00;
  TR0 = 0x00;  
}
